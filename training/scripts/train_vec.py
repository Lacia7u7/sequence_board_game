...
from training.envs.vectorized.subproc_vec_env import SubprocVecEnv
...
def maybe_auto_curriculum_vec(update_idx: int, meter, heur_names: List[str], old_w: Dict[str, float], cfg, vecenv: SubprocVecEnv, log=None) -> Dict[str, float]:
    cur = cfg.get("curriculum", {})
    if not cur or not bool(cur.get("enabled", False)):
        return old_w
    adjust_every = int(cur.get("adjust_every", 5))
    ...
    min_games = int(cur.get("min_games_for_class", 10))
    recent = meter.recent_winrates_by_class(min_games=min_games)
    new_w = _compute_curriculum_weights(heur_names, recent, old_w or {}, cur)
    try:
        vecenv.set_heuristic_weights(new_w)
        if bool(cur.get("log_weights", True)) and log is not None:
            for k, v in new_w.items():
                log.scalar(f"curriculum/heuristics/{k}", float(v), step=update_idx)
        print("[curriculum] heuristics weights @", update_idx, {k: round(v, 3) for k, v in new_w.items()})
    except Exception as e:
        print(f"[curriculum] failed to set weights: {e}")
        return old_w
    return new_w
...
def main():
    ...
    # ---- Build subprocess vectorized envs ----
    num_envs = int(cfg["training"].get("num_envs", 1))
    assert num_envs >= 1, "num_envs must be >= 1"
    num_workers = int(cfg["training"].get("num_workers", min(num_envs, os.cpu_count() or 1)))

    # (optional) gather existing snapshot files so worker pools can include them
    ...
    # NOTE: 'current' must be 0.0 for this minimal implementation (no live broadcast of learner weights to workers)
    if float(pool_probs.get("current", 0.0)) != 0.0:
        print(
            "[warning] pool_probabilities['current'] should be 0.0 when using SubprocVecEnv unless you add a weights broadcast")

    # If we’re resuming and want workers to preload snapshots, scan the run dir
    pre_snapshot_paths = []
    try:
        from ..utils.snapshot_manager import SnapshotManager
        run_dir = LoggingMux.get_run_dir(cfg)
        if os.path.isdir(run_dir):
            sm = SnapshotManager(run_dir, max_keep=max_snaps) if snapshot_every > 0 else None
            if sm is not None:
                pre_snapshot_paths = list(sm.all())
    except Exception:
        pass
    obs_i, info_i = SequenceEnv(cfg).reset()
    obs_shape = tuple(obs_i.shape)
    ...
    vecenv = SubprocVecEnv(
        ...
        pool_probabilities_description=pool_prob_desc,
        snapshot_paths=pre_snapshot_paths,
    )

    # First reset (already aligned to learner turn by workers)
    obs_np, info_list = vecenv.reset()
    obs_shape = tuple(obs_np.shape[1:])
    action_dim = int(np.asarray(info_list[0].get("legal_mask", np.ones((1,), np.float32))).shape[0])
    ...
    # Heuristic names (for curriculum weighting broadcast)
    heuristics_for_names = [
        GreedySequenceAgent(), CenterHeuristicAgent(), BlockingAgent(), RandomAgent(),
        BeamMinimaxAgent(), EnsembleHeuristicAgent(), ForkThreatAgent(),
        PatternWindowAgent(), ThreatAwareMinimaxAgent(), BonusProximityAgent(),
    ]
    heur_names = _pool_heuristic_names(type("H", (), {"heuristics": heuristics_for_names}))
    curr_old_weights: Dict[str, float] = {}

    # Prepare first obs tensor for learner
    obs_t = torch.from_numpy(obs_np).to(device, dtype=torch.float32)
    ...
    for update_idx in range(1, total_updates + 1):
        ...
        for t in range(rollout_len):
            ...
            # Step all envs in subprocesses (fast-forward to next learner turn inside workers)
            next_obs_np, next_info_list, reward_np, done_np, extras_list = vecenv.step(actions_np)

            # Meter updates for finished episodes (workers already reset/aligned internally)
            for ex in extras_list:
                if ex.get("episode_done", False):
                    meter.update(ex.get("opp_classes", []), bool(ex.get("learner_won", False)))

            # Pack tensors for storage at agent-turn sampling frequency
            obs_t = torch.from_numpy(next_obs_np).to(device, dtype=torch.float32)
            act_t = torch.from_numpy(actions_np).to(device, dtype=torch.long)
            rew_t = torch.from_numpy(reward_np).to(device, dtype=torch.float32)
            done_t = torch.from_numpy(done_np).to(device, dtype=torch.float32)
        ...
        curr_old_weights = maybe_auto_curriculum_vec(update_idx, meter, heur_names, curr_old_weights, cfg, vecenv,
                                                     log=log)
        ...
        if snapman is not None and snapshot_every > 0 and ((update_idx - 1) % snapshot_every) == 0:
            ...
            try:
                vecenv.add_snapshot(snap_path, max_keep=snapman.max_keep)
                print(f"[snapshots] added {snap_path} to all workers")
            except Exception as e:
                print(f"[snapshots] failed to broadcast to workers: {e}")
...
if __name__ == "__main__":
    ...
    mp.set_start_method("spawn", force=True)  # Windows & Colab safe
    main()
